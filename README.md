...
- `run_portable.bat` walks through Python environment validation, dependency installation, optional config bootstrapping, and finally launches the GUI with the current folder as the base directory.
- `create_portable_shortcut.bat` creates a desktop shortcut (`CryptoPortable.lnk`) pointing to `run_portable.bat`. Drop an icon named `crypto.ico` inside the `icons/` folder to style the shortcut automatically.
