...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Portable root directory (defaults to current working directory)",
    )
    parser.add_argument(
        "--config-dir",
        type=Path,
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Launch the graphical interface instead of running ingestion in CLI mode",
    )
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    paths = PortablePaths(base_dir=args.base_dir)
